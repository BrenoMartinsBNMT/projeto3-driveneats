...
function selecionarPedidoPrato(prato) {
  ...
  prato.classList.toggle("card-selecionado");
...
function trocarCorDoBotão() {
  if (pedidos.length >= 3) {
    ...
    corBotao.querySelector(".caixa-texto span").innerHTML = "Fechar Pedido";
  }
}
...
function confirmarPedido() {
  if (pedidos.length >= 3) {
    ...
    let BebidaNome = confirmarBebidas.querySelector(".nome-pedidos").innerText;
    let BebidaPreco =
    ...
    let sobremesaPreco =
      confirmarSobremesa.querySelector(".preço-sobremesa").attributes.value
        .value;
